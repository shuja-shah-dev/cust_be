...
def draw_detections(image, boxes, scores, class_ids, mask_alpha=0.3, mask_maps=None):
    ...
    mask_img, sum_pixels = draw_masks(image, boxes, class_ids, mask_alpha, mask_maps)
...
def draw_masks(image, boxes, class_ids, mask_alpha=0.3, mask_maps=None):
    ...
    for i, (box, class_id) in enumerate(zip(boxes, class_ids)):
        ...
        if mask_maps is None:
            ...
            sum_pixels.append((class_id, np.sum(crop_mask)))
            crop_mask_img = mask_img[y1:y2, x1:x2]
            crop_mask_img = crop_mask_img * (1 - crop_mask) + crop_mask * color
            mask_img[y1:y2, x1:x2] = crop_mask_img

    return cv2.addWeighted(mask_img, mask_alpha, image, 1 - mask_alpha, 0), sum_pixels
